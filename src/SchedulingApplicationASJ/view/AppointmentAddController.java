...
//    private final ObservableList<String> contacts = FXCollections.observableArrayList("jdoe", "rsmith", "ajohnson");
    ...
    public boolean handleAddAppointment(int id) {
        ...
        if(AppointmentMainController.overlappingAppointment(-1,location.getText(), ld.toString(), times.get(aptTime))) {
            errors.add("Overlapping Appointments");
            return false;
        }
        if(AppointmentMainController.saveAppointment(id, types.get(aptType), contact.getText(), location.getText(), ld.toString(), times.get(aptTime))) {
            return true;
        } else {
...
//    public void populateCstId(int id) {
//        cstId.setText(Integer.toString(id));
//    }
